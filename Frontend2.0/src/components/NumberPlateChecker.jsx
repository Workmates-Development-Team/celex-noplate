  ...
  const handleCheck = async () => {
    ...
    for (let i = 0; i < selectedImages.length; i++) {
      ...
      try {
        ...
        const textResponse =
          response.data.aggregatedResponse.candidates[0].content.parts[0].text;
        console.log("Response:", textResponse);
  
        // Extract Vehicle Numbers
        const vehicleNumbersMatch = textResponse.match(
          /\*\*\s*Vehicle number:\*\*\s*\[?([A-Z0-9]+),\s*([A-Z0-9]+)\]?/i
        );
        console.log("Vehicle Numbers:", vehicleNumbersMatch);
  
        // Extract Small Numbers
        const smallNumbersMatch = textResponse.match(
          /\*\*\s*Small number:\*\*\s*\[?([A-Z0-9]+),\s*([A-Z0-9]+)\]?/i
        );
        console.log("Small Numbers:", smallNumbersMatch);
  
        // Check for Hologram and Registration Matching
        const hologramMatch = textResponse.includes("hologram visible")
          ? "Yes"
          : "No";
        const registrationMatching = textResponse.includes(
          "both vehicle numbers are the same"
        )
          ? "Yes"
          : "No";
  
        // Extract values for JSON object fields
        const registrationno1 = vehicleNumbersMatch
          ? vehicleNumbersMatch[1].trim()
          : null;
        const registrationno2 = vehicleNumbersMatch
          ? vehicleNumbersMatch[2].trim()
          : null;
        const lid_no1 = smallNumbersMatch
          ? smallNumbersMatch[1].replace(/\s/g, "").substring(0, 12)
          : null;
        const lid_no2 = smallNumbersMatch
          ? smallNumbersMatch[2].replace(/\s/g, "").substring(0, 12)
          : null;
  
        // Auto-generate serial number
        const serialno = tableData.length + i + 1;
          ...
          <table className="w-full">
            <thead className="bg-gradient-to-r from-blue-600 to-red-600 text-white">
              <tr>
                ...
                <th className="py-3 px-6 text-left">LID Number 1</th>
                <th className="py-3 px-6 text-left">Registration No. 1</th>
                <th className="py-3 px-6 text-left">LID Number 2</th>
                <th className="py-3 px-6 text-left">Registration No. 2</th>
                {/* <th className="py-3 px-6 text-left">Registration Match</th> */}
              ...
              {tableData.length > 0 ? (
                tableData.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-6 border-b border-gray-200">
                      {item.registrationno1}
                    </td>
                    <td className="py-4 px-6 border-b border-gray-200">
                      {item.lid_no2}
                    </td>
                    <td className="py-4 px-6 border-b border-gray-200">
                      {item.registrationno2}
                    </td>
                    {/* <td className="py-4 px-6 border-b border-gray-200">
                      {item.registrationMatching}
                    </td> */}
                    <td className="py-4 px-6 border-b border-gray-200">
                      {item.hologram}
                    </td>
                ...
                <tr>
                  <td
                    colSpan="8" // Updated to match total columns including "Select All"
